...
class MyHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        clinvap = subprocess.run(
            ['nextflow', 'run', 'main.nf', '-w', WORK_DIR, '--skip_vep', 'true' '--annotated-vcf', os.path.abspath(event.src_path), '--genome', genome_assembly, '--outdir', DOWNLOADS, '-profile', 'parameters'], cwd=NEXTFLOW_FOLDER)
